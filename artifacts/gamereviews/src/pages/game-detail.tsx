...
export default function GameDetail() {
  ...
  const gameReviews = getGameReviews(game.id);
          ...
          <div className="flex-1 space-y-4">
            ...
            >
              ...
              <div className="flex gap-2 w-full sm:w-auto mt-4 sm:mt-0 flex-wrap">
                ...
                <Button variant="outline" className="font-bold border-primary text-primary hover:bg-primary hover:text-primary-foreground" onClick={() => {
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section id="review-section" className="space-y-6 pt-8 border-t border-border">
            <h3 className="text-3xl font-display font-bold flex items-center gap-3">
              ...
              <Badge variant="secondary" className="text-lg">{gameReviews.length}</Badge>
            </h3>
              ...
              <CardContent className="p-6 space-y-4 pt-8">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="recommend" className="text-sm font-medium leading-none cursor-pointer flex items-center gap-1">
                      <Shield className="h-4 w-4 text-primary" /> Lo recomiendo
                    </label>
                  </div>
                  
                  <div className="flex items-center gap-4">
                    <span className={`text-xs ${reviewText.length < 50 ? 'text-destructive' : 'text-green-500'}`}>
                      {reviewText.length}/50
                    </span>
                    <Button onClick={handlePublishReview} className="font-bold">Publicar Reseña</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-4 mt-8">
              {gameReviews.length === 0 ? (
                ...
                gameReviews.sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()).map(review => (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold flex items-center gap-2">
                              {review.autor}
                              {review.recomendado && (
                                <Badge variant="outline" className="bg-green-500/10 text-green-500 border-green-500/20 text-[10px] px-1 py-0 h-5">
                                  Recomendado
                                </Badge>
                              )}
                            </div>
          ...
          <Card className="bg-card border-border">
            <CardContent className="p-6 space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-border/50">
                <span className="text-muted-foreground flex items-center gap-2"><Calendar className="h-4 w-4"/> Lanzamiento</span>
                <span className="font-medium text-right">{new Date(game.fechaLanzamiento).toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-border/50">
                <span className="text-muted-foreground flex items-center gap-2"><Monitor className="h-4 w-4"/> Desarrollador</span>
                <span className="font-medium text-right">{game.desarrollador}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-border/50">
                <span className="text-muted-foreground flex items-center gap-2"><Tag className="h-4 w-4"/> Precio</span>
              ...
              <div className="pt-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {game.tags.map((t: string) => (
                    <span key={t} className="text-xs bg-background border border-border px-2 py-1 rounded-md text-muted-foreground">
                      #{t}
                    </span>
                  ))}
                </div>
          ...
          {game.skins && game.skins.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {game.skins.map((skin: any, i: number) => (
